...
class Solver_CT(object):
    def __init__(self, model, train_loader,val_loader,test_loader, args, device,penalty_mode):
        ...
        self.theta_label = np.linspace(0, 2 * np.pi, self.det_count, endpoint=False)#全视角
    ...
    def train(self):
        ...
        best_psnr_ds4 = 0
        best_psnr_ds2 = 0
        for epoch_i in range(self.start_epoch + 1, self.end_epoch + 1):#50轮
            ...
            for data in self.train_loader:
                ...
                theta = np.linspace(0, 2 * np.pi, int(self.det_count/ ds_theta), endpoint=False) # sparse views
                ...
                X_fbp = radon.backprojection(b_in) # FBP initialization
                ...
                batch_x = torch.abs(radon_label.backprojection(b_in_label)) # GT

                x_output = self.model(X_fbp, b_in,theta)

                # loss
                if self.loss_mode == 'L1':  # simple L1,MAE
                    ...
                    loss_all = loss_discrepancy + 0.1 * loss_discrepancy_k  # Compute and print loss

                elif self.loss_mode == 'L2':  # simple L1,MSE
            ...
            with torch.no_grad():
                ...
                for data in self.val_loader:
                    ...
                    x_output = model(X_fbp, b_in,theta)
                    end = time()
            ...
            with open(output_file_name, 'a') as output_file:
                if epoch_i == 1:
                    output_file.write(f"Training start time: {start_time}\n")

                output_data = [start_time, epoch_i, loss_all.item(), PSNR_mean, SSIM_mean, RMSE_mean]
